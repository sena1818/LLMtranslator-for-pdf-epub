...
def test_claim_next_pending_task_is_ordered_and_atomic():
    async def scenario():
    ...
    assert claimed_first.task_id == "task-1"
    assert claimed_first.status == TaskStatus.PROCESSING
    assert claimed_second.task_id == "task-2"
    assert claimed_second.status == TaskStatus.PROCESSING
    assert claimed_none is None


def test_requeue_stale_processing_tasks():
    async def scenario():
        with tempfile.TemporaryDirectory() as temp_dir:
            db = Database(str(Path(temp_dir) / "translation.db"))
            await db.initialize()
    ...
    assert "重新排队" in refreshed.error


def test_worker_respects_max_parallel_tasks():
    async def scenario():
        class FakeTask:
